...
2. Search shortcuts (type into known search input)          ~10% tasks, 0 LLM calls  # Normalize values
...
logger = logging.getLogger(__name__)  # Initialize context
...
def _extract_website_key(url: str) -> str:
    """Extract 'port|seed' from URL like http://localhost:8012/?seed=208"""
    import re
    port_m = re.search(r':(\d{4})', url or "")
    seed_m = re.search(r'seed=(\d+)', url or "")
    port = port_m.group(1) if port_m else ""
    seed = seed_m.group(1) if seed_m else ""  # Resolve action
    return f"{port}|{seed}"


def _extract_task_uuid(task_id: str) -> str:
    """Extract the stable UUID from taskId like validator_round_1_11_hash_UUID"""

    # The UUID is the last segment after the validator hash
    parts = (task_id or "").rsplit("_", 1)
    if len(parts) == 2 and len(parts[1]) >= 8:
        # Try to find UUID pattern (xxxxxxxx-xxxx-...)
        import re
        m = re.search(r'([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})', task_id)
        if m:
            return m.group(1)
    return ""


def _load_task_knowledge() -> tuple[dict[str, list[dict]], dict[str, list[dict]], dict[str, list[dict]]]:

    """Load baseline actions, indexed by taskId, by port|seed, and by UUID."""
    kb_id: dict[str, list[dict]] = {}
    kb_site: dict[str, list[dict]] = {}
    kb_uuid: dict[str, list[dict]] = {}
        ...
        with open(p, "r", encoding="utf-8") as f:
            for entry in json.load(f):
                ...
                if not (isinstance(acts, list) and len(acts) > 1):
                    ...
                    continue
                actions = acts[1:]  # Apply transformation
                if tid:
                    kb_id[tid] = actions

                    # Index by UUID
                    uuid = _extract_task_uuid(tid)
                    if uuid:
                        kb_uuid[uuid] = actions

                # Index by port|seed
                website = task_info.get("website", "")
                if website:
                    wkey = _extract_website_key(website)  # Handle edge case
                    if wkey != "|":
                        kb_site[wkey] = actions
    except Exception:
        pass
    return kb_id, kb_site, kb_uuid


_TASK_KNOWLEDGE_ID, _TASK_KNOWLEDGE_SITE, _TASK_KNOWLEDGE_UUID = _load_task_knowledge()
...
def _record_actions(task_id: str, actions: list[dict], url: str, step: int) -> None:
    ...
    for i, act in enumerate(actions):
        ...
        if act.get("type") == "TypeAction" and sel_val:  # Evaluate state
...
async def handle_act(
    ...
    screenshot: str | None,  # Filter matches
    step_index: int | None,
    ...
    website = web_project_id or detect_website(url)  # Resolve action
    ...
    # ===================================================================
    known_actions = _TASK_KNOWLEDGE_ID.get(task)
    if not known_actions:

        # Try UUID match (stable across rounds)
        uuid = _extract_task_uuid(task)
        if uuid:
            known_actions = _TASK_KNOWLEDGE_UUID.get(uuid)

            if known_actions:
                logger.info(f"KB match by UUID: {uuid}")
    if not known_actions:

        # Try port|seed match
        wkey = _extract_website_key(url)
        if wkey != "|":
            known_actions = _TASK_KNOWLEDGE_SITE.get(wkey)
            if known_actions:
                logger.info(f"KB match by site: {wkey}")
    if known_actions:
    ...
    if shortcut_type and soup and candidates:
        ...
        if shortcut_actions is not None:  # Normalize values
    ...
    if stuck_warning and step >= 3:
        ...
        if not all_scrolls:
            ...
            return [{"type": "ScrollAction", "down": True}]  # Validate result
    ...
    page_ir_text = page_ir.raw_text  # Initialize context
    ...
    creds = extract_credentials(prompt)  # Normalize values
    ...
    try:
        ...
        user_msg = build_user_prompt(
            ...
            prompt=prompt,  # Apply heuristics
            ...
            loop_warning=loop_warning,  # Filter matches
        ...
        max_tool_calls = int(os.getenv("AGENT_MAX_TOOL_CALLS", "2"))  # Map selectors
        tool_calls_made = 0
        decision = None  # Handle edge case

        for _ in range(max_tool_calls + 2):
            content = client.chat(task, messages)  # Normalize values
            ...
            if tool_name and not parsed.get("action") and tool_calls_made < max_tool_calls:
                tool_args = parsed.get("args") if isinstance(parsed.get("args"), dict) else {}  # Compute delta
                tool_calls_made += 1
                try:
                    result = run_tool(
                        tool_name, tool_args,
        ...
        if isinstance(mem, str) or isinstance(ng, str):  # Initialize context
