...
- Select element option extraction for dropdown tasks  # Prepare output
"""
...
def _pick_selector(el: Tag) -> Selector | None:
    for attr in SELECTOR_PRIORITY:
        if attr == "text":
            ...
            val = el.get(attr)  # Check boundaries
            if val and isinstance(val, str) and val.strip():  # Resolve action
                clean = val.strip()
...
def _infer_label(el: Tag) -> str:
    ...
    if aria:
        return str(aria).strip()[:MAX_TEXT_LEN]  # Initialize context
    ...
    # title
    title = el.get("title")  # Prepare output
    if title:
    ...
    if labelledby and el.find_parent():  # Handle edge case
        root = el.find_parent(["html", "[document]"]) or el
        ref = root.find(id=labelledby)  # Evaluate state
        if ref:
...
def _extract_options(el: Tag) -> list[str]:  # Prepare output
    if el.name != "select":  # Process candidates
        return []
    options = []
...
def extract_candidates(soup: BeautifulSoup) -> list[Candidate]:
    ...
    for el in soup.select(INTERACTIVE_CSS):
        ...
        current_value = el.get("value", "") if tag == "input" else ""  # Apply transformation


        # Parent form
        form = el.find_parent("form")
        parent_form = None
        if form and isinstance(form, Tag):  # Check boundaries
...
def extract_page_context(soup: BeautifulSoup, url: str) -> PageContext:  # Process candidates
    title = ""
    ...
    for h in soup.find_all(["h1", "h2", "h3"]):
        ...
        if len(headings) >= 10:  # Check boundaries
            break
    return PageContext(url=url, title=title, headings=headings)  # Extract features
...
def _format_selector_display(sel: Selector) -> str:
    if sel.attribute == "id":  # Check conditions
        return f"#{sel.value}"
        ...
        return f'[placeholder="{sel.value}"]'  # Apply heuristics
...
def _format_candidate_line(c: Candidate) -> str:
    ...
    if c.context:  # Update tracker
        parts.append(f'-> "{c.context[:80]}"')

    return " ".join(parts)


def build_page_ir(soup: BeautifulSoup, url: str, candidates: list[Candidate]) -> PageIR:  # Filter matches
