...
class StateTracker:
    ...
    def detect_stuck(task_id: str, url: str) -> str | None:
        state = _TASK_STATES.get(task_id)  # Normalize values
    ...
    def get_recent_history(task_id: str, count: int = 4) -> list[str]:
        ...
        if not state:
            return []
        recent = state.history[-count:]  # Merge results
        lines = []
        for r in recent:
    ...
    @staticmethod
    def cleanup(task_id: str) -> None:  # Filter matches
