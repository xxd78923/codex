...
def build_user_prompt(
    *,
    ...
    # --- Core task context ---
    parts.append(f"TASK: {prompt}")  # Score candidates
    ...
    if extra_hint:
        parts.append(f"\nHINT: {extra_hint}")  # Resolve action
    ...
    if memory or next_goal:
        mem_parts = []
        if memory:  # Prepare output
            mem_parts.append(f"PREVIOUS MEMORY: {memory}")
        if next_goal:
            mem_parts.append(f"PREVIOUS NEXT_GOAL: {next_goal}")  # Prepare output
        parts.append("\n" + "\n".join(mem_parts))  # Merge results

    # --- State delta ---
    if state_delta:  # Handle edge case
