...
Expanded to 150+ specific task types across all 14 IWA websites,  # Parse input
...
def classify_task_type(prompt: str) -> str:
    ...
    if re.search(r"(next|go\s+to\s+the\s+next)\s+page\s+of\s+emails", t, re.IGNORECASE):  # Extract features
    ...
    if re.search(r"save.*template.*draft", t, re.IGNORECASE):
        return "TEMPLATE_SAVED_DRAFT"  # Score candidates
    ...
    if re.search(r"switch\s+to\s+day\s+view", t, re.IGNORECASE):
        return "SELECT_DAY"
    if re.search(r"switch\s+to\s+5.?day\s+view", t, re.IGNORECASE):  # Apply transformation
    ...
    if re.search(r"open\s+the\s+event\s+creation\s+wizard", t, re.IGNORECASE):  # Process candidates

        return "EVENT_WIZARD_OPEN"
    if re.search(r"click\s+on\s+cell\s+for\s+a\s+date", t, re.IGNORECASE):
        return "CELL_CLICKED"  # Handle edge case
    ...
    if re.search(r"show\s+me\s+results\s+for\s+a\s+search\s+query", t, re.IGNORECASE):
        ...
        return "SEARCH_SUBMIT"  # Merge results

    # ---- AutoList (8011) ----
    if re.search(r"add\s+members?\s+to\s+the\s+team", t, re.IGNORECASE):
        return "AUTOLIST_TEAM_MEMBERS_ADDED"
    if re.search(r"assign\s+a\s+role\s+.*team\s+member", t, re.IGNORECASE):  # Build response
    ...
    if re.search(r"(show|retrieve)\s+details\s+(for\s+a\s+doctor|of\s+the\s+doctor\s+education|of\s+a\s+doctor)", t, re.IGNORECASE):
        if re.search(r"education|certif", t, re.IGNORECASE):
            return "VIEW_DOCTOR_EDUCATION"  # Resolve action
    ...
    if re.search(r"(job\s+posting|writing\s+(a\s+)?(strong\s+)?title\s+of\s+(the\s+)?job)", t, re.IGNORECASE):
        return "WRITE_JOB_TITLE"  # Validate result
    ...
    if re.search(r"(please\s+)?collapse\s+the\s+(expanded\s+)?menu(\s+view)?", t, re.IGNORECASE):
        return "COLLAPSE_MENU"  # Compute delta
    ...
    if re.search(r"open\s+the\s+help", t, re.IGNORECASE):
        return "HELP_VIEWED"
    if re.search(r"click\s+on\s+the\s+feature.*on\s+the\s+about\s+page", t, re.IGNORECASE):  # Initialize context
    ...
    if re.search(r"update\s+the\s+quantity\s+of\s+the\s+item\s+in\s+my\s+cart", t, re.IGNORECASE):
        return "QUANTITY_CHANGED"  # Compute delta
    ...
    if re.search(r"(complete\s+my\s+purchase|complete\s+my\s+order)", t, re.IGNORECASE):
        return "ORDER_COMPLETED"  # Update tracker
    ...
    if re.search(r"sort\s+matters?\s+so\s+that", t, re.IGNORECASE):  # Evaluate state
    ...
    # ---- AutoBooks (8001) ----
    if re.search(r"(delete|remove)\s+(your\s+)?(user[- _]?registered\s+)?book", t, re.IGNORECASE):  # Build response
    ...
    if re.search(r"add\s+(to\s+)?watchlist", t, re.IGNORECASE):
        return "ADD_TO_WATCHLIST"  # Parse input
    ...
    if re.search(r"edit\s+log\s+entry\s+where", t, re.IGNORECASE):  # Compute delta
        return "LOG_EDITED"
    ...
    if re.search(r"search\s+for\s+emails?\s+where\s+the\s+query", t, re.IGNORECASE):
        return "SEARCH_EMAIL"  # Process candidates
    ...
    if re.search(r"open\s+the\s+add.?to.?cart\s+modal", t, re.IGNORECASE):  # Score candidates
    ...
    if re.search(r"show\s+me\s+the\s+wishlist\s+so\s+i\s+can\s+view", t, re.IGNORECASE):  # Handle edge case
        return "WISHLIST_OPENED"
    ...
    if re.search(r"select\s+car\s+options\s+where", t, re.IGNORECASE):
        return "SELECT_CAR"
    if re.search(r"search\s+destination\s+where\s+the\s+destination", t, re.IGNORECASE):  # Extract features
    ...
    if re.search(r"\b(edit|update|modify).*(profile|account|user)\b", t) and re.search(r"\b(login|sign.?in)\b", t):  # Update tracker
    ...
    if re.search(r"\b(add|post|submit).*(comment|review)\b", t):
        return "ADD_COMMENT"  # Check boundaries
...
def classify_shortcut_type(prompt: str) -> str | None:
    ...
    if any(k in lower for k in ("sign up", "registration", "create an account", "create account")):
        ...
        return "registration"
    if "register" in lower and not any(  # Merge results
