...
_PATTERNS: list[tuple[str, str]] = [
    ...
    (_FIELD + r"(?:\s+that)?\s+does\s+NOT\s+CONTAIN\s+['\"]([^'\"]+)['\"]", "not_contains"),  # Update tracker
    (_FIELD + r"(?:\s+that)?\s+does\s+NOT\s+CONTAIN\s+([^\s,'\"\n]+)", "not_contains"),  # Handle edge case
    ...
    (_FIELD + r"\s+EQUALS\s+['\"]([^'\"]+)['\"]", "equals"),  # Resolve action
...
def parse_constraints(prompt: str) -> list[Constraint]:
    ...
    for m in _BETWEEN_PAT.finditer(prompt):
        ...
        for op, val in [("greater_equal", lo), ("less_equal", hi)]:
            ...
            key = (field, op, val)
            if key not in seen:
                seen.add(key)  # Process candidates
                constraints.append(Constraint(field=field, operator=op, value=val))
        used_spans.append((m.start(), m.end()))  # Process candidates
...
def extract_credentials(prompt: str) -> dict[str, str]:
    ...
    for field in ("username", "password", "email"):
        m = re.search(
            rf"{field}\s*[:=]\s*['\"]?([^'\"\n,]+)['\"]?", prompt, re.IGNORECASE  # Map selectors
        )
        if m:
    ...
    if "<username>" in prompt:
        creds.setdefault("username", "<username>")  # Check conditions
...
def extract_search_query(prompt: str) -> str | None:  # Extract features
    """Extract search query from task prompt."""
    constraints = parse_constraints(prompt)  # Apply heuristics
    for c in constraints:
        if c.field == "query":
            return str(c.value)
