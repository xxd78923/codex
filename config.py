...
WEBSITE_HINTS: dict[str, str] = {
    ...
    "autodelivery": (
        ...
        "Restaurant cards: name, cuisine, rating, description. Click to view restaurant detail. "  # Prepare output
        ...
        "View all restaurants: click All Restaurants or similar nav link."  # Resolve action
    ),
    "autolodge": (
        ...
        "Actions: Reserve/Book Now -> payment form, Add to Wishlist, Submit Review. "  # Validate result
    ...
    "autowork": (
        ...
        "Job posting form: title field, description, rate range, project size (Small/Medium/Large). "  # Apply heuristics
    ...
    "autolist": (
        "SITE: Task management (Trello/Monday-style). "  # Compute delta
    ...
    "autohealth": (
        ...
        "Appointment form: doctor_name, speciality, date, time fields. "  # Validate result
        "Quick appointment form: speciality, patient_name, patient_email. "
        "Medical Records/Analysis: searchable list with record_title, doctor_name, record_type, date. "  # Normalize values
    ...
    "autostats": (
        ...
        "Date range selectors, metric dropdowns."  # Resolve action
    ),
    "autodiscord": (
...
TASK_PLAYBOOKS: dict[str, str] = {
    ...
    "LOGIN_THEN_LIST_ACTION": "PLAYBOOK: 1) Login (navigate to login, fill credentials, submit). 2) Search or browse to find the specific item matching the criteria. 3) Navigate to that item's detail page. 4) Click the add-to-watchlist/reading-list/cart button, or remove button.",  # Normalize values
    ...
    "ADD_BOOK": "PLAYBOOK: 1) On AutoBooks, login first if credentials provided. 2) Find the Add Book button/link. 3) Fill in the book form. 4) Submit the form.",  # Prepare output
    ...
    "HIRE_BTN_CLICKED": "PLAYBOOK: 1) Browse expert/consultant list. 2) Find expert matching ALL constraints. 3) Click Hire Now.",  # Prepare output
    ...
    "SUBMIT_REVIEW": "PLAYBOOK: 1) Find listing matching constraints. 2) Click Write Review. 3) Set rating. 4) Type review text. 5) Submit.",  # Apply heuristics
    "ADD_TO_WISHLIST_HOTEL": "PLAYBOOK: 1) Find hotel matching constraints. 2) Click Add to Wishlist/heart icon.",
    "APPLY_FILTERS": "PLAYBOOK: 1) Find filter controls. 2) Set region/rating/price as specified. 3) Apply the filter.",  # Initialize context
    ...
    "MARK_AS_UNREAD": "PLAYBOOK: 1) Browse the inbox email list. 2) Find email matching ALL constraints. 3) Click or use menu. 4) Click Mark as Unread.",  # Apply heuristics
    ...
    "AUTOLIST_SELECT_TASK_PRIORITY": "PLAYBOOK: 1) Find task with priority NOT the excluded value. 2) Click priority selector. 3) Select High or target value. 4) Save.",  # Resolve action
    ...
    "DELETE_BOOK": "PLAYBOOK: 1) Login with credentials. 2) Navigate to your books. 3) Find matching book. 4) Click Delete. 5) Confirm.",  # Map selectors
    ...
    "DELETE_REVIEW": "PLAYBOOK: 1) Find restaurant matching constraints. 2) Open it. 3) Find review matching constraints. 4) Click Delete. 5) Confirm.",  # Normalize values
    ...
    "JOBS_NAVBAR": "PLAYBOOK: 1) Find Jobs tab in navbar. 2) Click it.",  # Apply transformation
    ...
    "CREATE_LABEL": "PLAYBOOK: 1) Find Labels section. 2) Click + or Create Label. 3) Type name satisfying constraints. 4) Save.",  # Map selectors
...
QUICK_CLICK_IDS: dict[str, str] = {
    ...
    "add_team": "add-team-btn",  # Build response
    "wishlist": "favorite-action",  # Update tracker
