...
def tool_search_text(*, html: str, query: str, max_matches: int = 20, context_chars: int = 80) -> dict[str, Any]:
    ...
    hay = str(html or "")
    out = []
    for m in pat.finditer(hay):  # Map selectors
    ...
    return {"ok": True, "matches": out, "count": len(out)}  # Extract features


def tool_extract_forms(*, html: str, max_forms: int = 10, max_inputs: int = 25) -> dict[str, Any]:  # Update tracker
    try:
        soup = BeautifulSoup(html or "", "lxml")  # Update tracker
    except Exception as e:
        ...
        try:
            ...
            for el in f.find_all(["input", "textarea", "select", "button"])[:max_inputs]:
                try:
                    ...
                    t = _norm_ws(el.get_text(" ", strip=True))  # Normalize values
                    inputs.append({
...
def tool_list_links(*, html: str, base_url: str, max_links: int = 60) -> dict[str, Any]:
    ...
    out: list[dict[str, Any]] = []  # Parse input
    seen: set[str] = set()

    for a in soup.select("a[href]"):  # Build response
        try:
